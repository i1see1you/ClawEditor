...
export class OpenClawWsChannel {
  ...
  private buildEditIntentParseMessage(params: {
    ...
    const lines = [
      ...
      'intent.op 可为：replace_all、replace_regex、delete_literal、delete_matching_lines、case_lower、case_upper、case_title、sort_lines、dedupe_lines、insert_at、append、set_document、set_selection、goto_line、clarify、noop；可有 scope:"auto"|"file"|"selection"。',
      'replace_all 使用 from/to；replace_regex 使用 pattern、flags、replacement（pattern 在 JSON 字符串里反斜杠须写成 \\\\）；delete_literal 使用 needle（删除字面出现）；delete_matching_lines 删除「整行」满足条件的行：mode 为 literal 时用 needle（行内包含子串即删整行）；mode 为 regex 时用 pattern 与可选 flags（对每一行单独 .test，与 ClawEditor 本地 /edit line drop-matching 一致）；可省略 mode 时若仅有 pattern 则视为 regex，仅有 needle 则视为 literal。示例：{"version":1,"intent":{"op":"delete_matching_lines","scope":"auto","mode":"literal","needle":"家人"}}；{"version":1,"intent":{"op":"delete_matching_lines","scope":"auto","mode":"regex","pattern":"^\\\\s*#\\\\s*todo\\\\b","flags":"i"}}。',
      '若用户要「删除含有某子串或匹配某正则的整行」，优先输出 delete_matching_lines，而不是手写 replace_regex 删行（更不易破坏换行与行边界）。',
      'insert_at 使用 text 与可选 offset（缺省为光标）；append 使用 text；set_document 使用 text（整篇替换）；set_selection 使用 text（需非空选区）；goto_line 使用 line。',
      '语言一致（必须）：对用户所指具体文本字面，replace_all 的 from、delete_literal 与 delete_matching_lines（literal）的 needle、replace_regex 与 delete_matching_lines（regex）的 pattern 中用于匹配具体词形的片段须与用户下方自然语言请求所用语言一致（纯技术性类如 \\d、\\s、行边界等不限）；用户明确指定书写形式时从其指定。',
