...
export class OpenClawWsChannel {
  ...
  private buildEditIntentParseMessage(params: {
    ...
    const lines = [
      ...
      'intent.op 可为：replace_all、replace_regex、delete_literal、case_lower、case_upper、case_title、sort_lines、dedupe_lines、insert_at、append、set_document、set_selection、goto_line、clarify、noop；可有 scope:"auto"|"file"|"selection"。',
      'replace_all 使用 from/to；replace_regex 使用 pattern、flags、replacement（pattern 在 JSON 字符串里反斜杠须写成 \\\\）；delete_literal 使用 needle；insert_at 使用 text 与可选 offset（缺省为光标）；append 使用 text；set_document 使用 text（整篇替换）；set_selection 使用 text（需非空选区）；goto_line 使用 line。',
      '语言一致（必须）：对用户所指具体文本字面，replace_all 的 from、delete_literal 的 needle、replace_regex 的 pattern 中用于匹配具体词形的片段须与用户下方自然语言请求所用语言一致（纯技术性类如 \\d、\\s、行边界等不限）；用户明确指定书写形式时从其指定。',
